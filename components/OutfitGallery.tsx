  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-6 py-4 flex justify-between items-center border-b border-gray-100">
        <h2 className="text-sm font-medium uppercase tracking-widest text-gray-400">灵感搭配 ({outfits.length})</h2>
        <button 
          onClick={onCreateNew}
          className="bg-black text-white px-4 py-2 rounded-full text-xs active:scale-95 transition-transform"
        >
          + 新建搭配
        </button>
      </div>
                ...
                >
                  <div className="w-full h-full grid grid-cols-2 grid-rows-2 gap-0.5 p-0.5">
                    ...
                    {outfitItems.length < 4 && Array.from({ length: 4 - outfitItems.length }).map((_, i) => (
                      <div key={i} className="bg-gray-50 flex items-center justify-center text-[8px] text-gray-200 font-bold italic">ITEM</div>
                    ))}
                  </div>
                  
                  <div className="absolute inset-x-0 bottom-0 bg-white/95 p-3 flex flex-col justify-end translate-y-1 group-hover:translate-y-0 transition-transform">
                    <p className="text-black text-[10px] font-medium uppercase italic tracking-tighter truncate">{outfit.name}</p>
                    <p className="text-gray-400 text-[8px] font-normal uppercase tracking-widest">{outfitItems.length} 件单品</p>
                  </div>
                </div>
              );
            })}
      ...
      {selectedOutfit && (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-0 sm:p-6">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-t-3xl sm:rounded-3xl overflow-hidden animate-slide-up max-h-[90vh] flex flex-col shadow-2xl">
            <div className="p-4 border-b flex justify-between items-center bg-white sticky top-0 z-10">
              <button onClick={() => setSelectedOutfit(null)} className="text-gray-400 text-xs px-2 uppercase tracking-widest font-medium">返回</button>
              ...
              {isEditingName ? (
                <div className="flex-1 px-4">
                  <input 
                    ...
                    className="w-full font-medium text-sm border-b border-black outline-none py-1 text-black"
                  />
                </div>
              ) : (
                <h3 
                  className="font-medium text-sm flex items-center gap-1 cursor-pointer text-black uppercase tracking-tighter"
              ...
              <button 
                ...
                className="text-red-500 text-[10px] font-medium px-2 uppercase tracking-widest"
              >删除</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div>
                <h4 className="text-[10px] font-medium text-gray-300 uppercase tracking-[0.3em] mb-4 flex items-center gap-3">
                  包含单品
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                  {getOutfitItems(selectedOutfit).map(item => (
                    <div key={item.id} className="space-y-2 text-center group">
                      ...
                      <p className="text-[8px] font-medium text-gray-400 truncate px-1 uppercase tracking-tighter group-hover:text-black">{item.name}</p>
                    </div>
                  ))}
                </div>
              </div>
